...
CommandPart::CommandPart(std::vector<std::string> channels, std::string message)
...
void CommandPart::execute(Client *client, const Server &server)
{
  ...
  for (std::set<Channel *>::iterator it = channels.begin(); it != channels.end(); ++it) {
	  Channel *ch = *it;
      for (std::vector<std::string>::iterator it2 = this->channels.begin();
           it2 != this->channels.end(); ++it2)
        {
          if (ch->getName() == *it2) {
              ch->publishMessage(
                  ":" + client->getNickname() + "!" + client->getUser()->getUsername() +
                      "@ PART #" + ch->getName() + " :" + message,
              ...
              DEBUG_LOG(
                  "User " << client->getNickname() << " has left channel " << (*it)->getName()
              );
            }
        }
    }
