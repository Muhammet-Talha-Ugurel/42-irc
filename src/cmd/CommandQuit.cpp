...
void CommandQuit::execute(Client *client, const Server &server)
{
  ...
  for (std::set<Channel *>::iterator it = channels.begin(); it != channels.end(); ++it)
    (*it)->publishMessage(
        ":" + client->getNickname() + "!" + client->getUser()->getUsername() + "@ QUIT :" + message, client,
...
void CommandQuit::execute(const Client *client, const Server &server) { execute(const_cast<Client *>(client), server); }
